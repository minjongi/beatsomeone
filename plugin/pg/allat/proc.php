...
$at_amt=$_SESSION['TOTAL_PRICE'];						//결제 금액을 다시 계산해서 만들어야 함(해킹방지), ( session, DB 사용 )
$lang_cd=$_SESSION['LANG_CD'];

if ($lang_cd == 'en') {
    $completeUrl = '/payment-complete';
} else {
    $completeUrl = '/' . $lang_cd . '/payment-complete';
}
...
//    echo "부분취소가능여부 : ".$PARTCANCEL_YN."<br>";
    ...
    <script>
        jQuery.ajax({
            ...
            data: {'action': 'complete_order', 'id': '<?= $ORDER_NO ?>'},
            type: 'get',
            dataType: 'json',
            async: true
        }).done(function (ret) {
            opener.location.replace('<?= $completeUrl ?>');
            self.close();
        });
    </script>
    <?php
    ...
    ?>
    <script>
        alert('<?= $REPLYCD . ' : ' . iconv('euc-kr', 'utf-8', $REPLYMSG) ?>');
