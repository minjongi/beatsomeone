 ...
 */
class Error404 extends CB_Controller
{
	/**
	 * 모델을 로딩합니다
	 */
    ...
    {
        if ($this->uri->total_segments() > 0 && $this->uri->total_segments() < 3) {
            $segs = $this->uri->segment_array();
            $nickname = array_key_exists($segs[1], $this->config->item('validLocale')) ? $segs[2] : $segs[1];
        }

        if (
            !empty($nickname) &&
            !preg_match("/[\,]?{$nickname}/i", $this->cbconfig->item('denied_nickname_list'))
        ) {
            $userinfo = $this->Member_model->get_by_nickname($nickname);
            if (!empty($userinfo['mem_id'])) {
                $this->shop($userinfo);
                return;
            }
        }
