...
class Allat extends CB_Controller
{
    ...
    public function proc()
    {
        ...
        //----------------------
        $at_cross_key = "55a7131be375ad9962aeb64081486950";    //설정필요 [사이트 참조 - http://www.allatpay.com/servlet/AllatBiz/support/sp_install_guide_scriptapi.jsp#shop]
        $at_shop_id = "levoist";        //설정필요
...
//        if (!strcmp($REPLYCD, "0000")) {
            ...
            // @TODO : wextown 주문 결제 완료처리

            $this->session->set_userdata(
                'order_complete_token',
                $SEQ_NO
            );
        } else {
        ...
        <script>
            if(typeof(opener) != "undefined") {
                opener.result_submit('<?= $REPLYCD ?>', '<?= iconv('euc-kr', 'utf-8', $REPLYMSG) ?>', '<?= $_POST["allat_enc_data"] ?>');
                self.close();
            } else {
                result_submit('<?= $REPLYCD ?>', '<?= iconv('euc-kr', 'utf-8', $REPLYMSG) ?>', '<?= $_POST["allat_enc_data"] ?>');
            }
            //opener.document.fpayment.allat_enc_data.value = '<?//= $_POST["allat_enc_data"] ?>//';
            //opener.result_submit('<?//= $REPLYCD ?>//', '<?//= iconv('euc-kr', 'utf-8', $REPLYMSG) ?>//');
            //self.close();
        </script>
        <?php
        /*
