...
class Managelayout extends CI_Controller
{
	...
	function front($config = array(), $device_view_type = '')
	{
        ...
        $data['og_image'] = element('og_image', $config) ?? site_url() . 'assets/images/common/thumb_sns1.png';
    ...
    function display_script()
    {
        ...
        $_script = $this->script;
        if ($_script) {
            foreach ($_script as $val) {
                $return .= '<script type="text/javascript">' . $val . '</script>' . PHP_EOL;
            }
        }
        return $return;
    }
