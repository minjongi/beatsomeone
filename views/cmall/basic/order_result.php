...
<table class="table table-hover mt20">
  ...
  if (element('orderdetail', $view)) {
      foreach (element('orderdetail', $view) as $result) {
          ?>
        <tr>
          <td><a href="<?php echo cmall_item_url(element('cit_key', element('item', $result))); ?>"
                 title="<?php echo html_escape(element('cit_name', element('item', $result))); ?>"><img
                  src="<?php echo thumb_url('cmallitem', element('cit_file_1', element('item', $result)), 60, 60); ?>"
                  class="thumbnail" style="margin:0;width:60px;height:60px;"
                  alt="<?php echo html_escape(element('cit_name', element('item', $result))); ?>"
                  title="<?php echo html_escape(element('cit_name', element('item', $result))); ?>"/></a></td>
          <td colspan="2"><a href="<?php echo cmall_item_url(element('cit_key', element('item', $result))); ?>"
                             title="<?php echo html_escape(element('cit_name', element('item', $result))); ?>"><?php echo html_escape(element('cit_name', element('item', $result))); ?></a>
            <ul class="cmall-options">
                ...
                foreach (element('itemdetail', $result) as $detail) {
                  ...
                  <li><?php echo html_escape(element('cde_title', $detail)) . ' ' . element('cod_count', $detail); ?>개
                    (+<?php echo number_format(element('cde_price', $detail)); ?>원)
                      <?php
                      if (element('cor_status', element('data', $view)) === '1') {
                          if (element('possible_download', element('item', $result))) {
                              ?>
                            <a href="<?php echo site_url('cmallact/download/' . element('cor_id', element('data', $view)) . '/' . element('cde_id', $detail)); ?>"
                               type="button" name="download" class="btn btn-xs btn-black pull-right">다운로드</a>
                          <?php } else { ?>
          ...
          <td><?php echo number_format($total_price); ?><input type="hidden"
                                                               name="total_price[<?php echo element('cit_id', element('item', $result)); ?>]"
...
<div class="credit">
  <table class="table table-striped table-bordered table-hover">
    ...
    <tr>
      ...
      <td>
          <?php echo element('cor_id', element('data', $view));
          if (element('is_test', element('data', $view))) { ?>
    ...
    <?php } ?>
    <?php if (strcasecmp(element('cor_pay_type', element('data', $view)), 'card') == 0 || strcasecmp(element('cor_pay_type', element('data', $view)), '3d') == 0) { ?>
      <tr>
    ...
    <?php if (strcasecmp(element('cor_pay_type', element('data', $view)), 'card') == 0 or strcasecmp(element('cor_pay_type', element('data', $view)), '3d') == 0) {
        if (element('is_test', element('data', $view)) == '0') {
            ?>
          <tr>
            <td class="text-center">전표</td>
            <td>
                <?php
                if (strcasecmp(element('cor_pay_type', element('data', $view)), 'card') == 0 or strcasecmp(element('cor_pay_type', element('data', $view)), '3d') == 0) {
                    if (element('is_test', element('data', $view)) == '0') {
                        $receipturl = 'http://www.allatpay.com/servlet/AllatBizPop/member/pop_card_receipt.jsp?shop_id=' . $this->cbconfig->item('allat_shop_id') . '&order_no=';
                    } else {
                        $receipturl = 'http://www.allatpay.com/servlet/AllatBizPop/member/pop_card_receipt.jsp?shop_id=' . $this->cbconfig->item('allat_shop_id') . '&order_no=';
                    }
                    ?>
                  <a href="javascript:;"
                     onclick="window.open('<?php echo $receipturl . element('cor_id', element('data', $view)); ?>', 'app', 'width=410,height=650,scrollbars=0')"
                     title="전표 출력">신용카드 전표 출력</a>
                <?php } ?>
            </td>
          </tr>
            <?php
  ...
  <table class="table table-striped table-bordered table-hover">
    ...
    <tr>
      ...
      <td><?php echo number_format((int)element('cor_total_money', element('data', $view))); ?> 원</td>
    </tr>
    <?php if (element('cor_deposit', element('data', $view))) { ?>
      <tr>
        <td class="table-left"><?php echo html_escape($this->cbconfig->item('deposit_name')); ?>사용</td>
        <td><?php echo number_format((int)element('cor_deposit', element('data', $view))); ?> 원</td>
      </tr>
    <?php }
    $notyet = element('cor_cash_request', element('data', $view)) - element('cor_cash', element('data', $view)); ?>
    <tr class="info">
      <td class="table-left">결제액</td>
      <td><?php echo number_format((int)element('cor_cash', element('data', $view))); ?> 원 <?php if (!$notyet) {
              echo '(완납)';
          } ?></td>
    </tr>
    <?php if ($notyet) { ?>
      <tr>
        ...
        <td><?php echo number_format((int)$notyet); ?> 원</td>
      </tr>
    <?php } ?>
    </tbody>
  </table>
