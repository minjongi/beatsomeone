    ...
    <div class="row" style="display:flex; margin-bottom:30px;">
      <div class="tabmenu">
        <div
          ...
          @click="goTabMenu(1)"
        >{{$t('totalQuantity')}} ({{calcTotalCnt}})</div>
        <div
          :class="{ 'active': search_tabmenu_idx === 2 }"
          @click="goTabMenu(2)"
        >{{$t('selling')}} ({{calcSellingCnt}})</div>
        <div
          :class="{ 'active': search_tabmenu_idx === 3 }"
          @click="goTabMenu(3)"
        >{{$t('pending')}} ({{calcPendingCnt}})</div>
      </div>
      <div>
        <div class="sort">
          <span>{{ $t('sortBy') }}</span>
          ...
          >
            <button
              ...
              @click="toggleGMT"
            >{{$t('genre')}} / {{$t('mood')}} / {{$t('trackType')}}</button>
                ...
                <figure>
                  ...
                  <figcaption class="pointer">
                    <div class="info">
                      ...
                      <div v-if="item.cit_status === '1'" class="status SEELING">{{$t('selling1')}}</div>
                      <div v-if="item.cit_status === '0'" class="status PENDING">{{$t('pending1')}}</div>
