        ...
        <div class="row" style="margin-bottom:10px;">
            <div class="search condition">
                <div class="filter">
                    <div
                        class="condition"
                        :class="{ 'active': search_condition_active_idx === 1 }"
                        @click="setSearchCondition(1)"
                    >{{ $t('productName') }}
                    </div>
                    <div
                        class="condition"
                        :class="{ 'active': search_condition_active_idx === 2 }"
                        @click="setSearchCondition(2)"
                    >{{ $t('productCode') }}
                    </div>
                    <div
                        class="condition"
                        :class="{ 'active': search_condition_active_idx === 3 }"
                        @click="setSearchCondition(3)"
                    >{{ $t('keyword') }}
                    </div>
                </div>
                <div class="wrap">
                    <input
                        type="text"
                        :placeholder="$t('searchingProduct')"
                        :value="goSearchText"
                        @input="goSearchText=$event.target.value"
        ...
        <div class="row count-sort">
            <div class="tabmenu">
                <div
                    ...
                    @click="goTabMenu(1)"
                >{{ $t('totalQuantity') }} ({{ calcTotalCnt }})
                </div>
                <div
                    :class="{ 'active': search_tabmenu_idx === 2 }"
                    @click="goTabMenu(2)"
                >{{ $t('selling') }} ({{ calcSellingCnt }})
                </div>
                <div
                    :class="{ 'active': search_tabmenu_idx === 3 }"
                    @click="goTabMenu(3)"
                    ...
                    >
                                ...
                                <figure>
                                    ...
                                    <figcaption class="pointer">
                                        <div class="info">
                                            ...
                                            <div v-if="item.cit_status === '1'" class="status SEELING">
                                                {{ $t('selling1') }}
                                            </div>
                                            <div v-if="item.cit_status === '0'" class="status PENDING">
                                                {{ $t('pending1') }}
                                            </div>
                                        ...
                                        <div class="listen">
                                            <div class="playbtn">
                                                <button
                                                    ...
                                                    :id="'playAction' + item.cit_id "
                                                >재생
                                                </button>
                                                <span class="timer"><span data-v-27fa6da0 class="current">0:00 /</span><span
                        ...
                        <span class="option_fold">
                          <img src="/assets/images/icon/togglefold.png" @click.self="toggleButton"/>
                                            ...
                                            <div class="price">{{ formatPrice(item.cde_price, item.cde_price_d, true) }}
                                            </div>
                                        </div>
                                        <!-- BASIC LEASE LICENSE -->
                                        ...
                                        <div class="n-box" v-else-if="item.cit_mastering_license_use === '1' ">
                                            ...
                                            <div class="price">{{
                                                    formatPrice(item.cde_price_2, item.cde_price_d_2, true)
                                                }}
                                            </div>
                                        </div>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        async ajaxUserInfo() {
            try {
                this.isLoading = true;
                // const {data} = await axios.get("/beatsomeoneApi/get_user_info", {});
            ...
            this.ajaxItemList().then(() => {
                ...
                if (this.search_condition_active_idx == 1) {
                    let rst = list.filter((item) =>
                        this.formChange(item.cit_name).includes(
                            this.formChange(this.goSearchText)
                        )
                    );
                    this.myProduct_list = rst;
                } else if (this.search_condition_active_idx == 2) {
                    let rst = list.filter((item) =>
                        this.formChange(item.cit_key).includes(
                            this.formChange(this.goSearchText)
                        )
                    );
                    this.myProduct_list = rst;
                    ...
                    let rst = list.filter((item) =>
                        this.formChange(item.hashTag).includes(
                            this.formChange(this.goSearchText)
                        )
                    );
                    this.myProduct_list = rst;
